...
![Coverage gate](https://img.shields.io/badge/coverage%20gate-%E2%89%A564%25-green)
...
（332 条样本）与 575 项自动化测试，CI 在 Windows / Ubuntu 双系统执行。文档体系覆盖
...
tests/             Pytest suite (49 files, 575 tests, per-module coverage)
...
| `WORKSPACE_ALLOWED_ROOTS` | Directories the workspace switcher may open | Project root + user home are allowed |
| `DATABASE_PATH`, `OUTPUTS_DIR`, `KNOWLEDGE_FILES_DIR`, `LOGS_DIR` | Relocate runtime data (containers, mounted volumes) | Defaults under the project root |
...
575 tests across 49 files, one dedicated file per major module:
...
- **Backend tests** on an `ubuntu-latest` / `windows-latest` matrix with a coverage
  floor (`--cov-fail-under=64`), so regressions fail the build on both platforms.
