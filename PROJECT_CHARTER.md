...
当前已具备：多 Agent 编排（LangGraph）、动态任务队列、9 个工具（插件化注册）、
...
- Browser / MCP / Database 三个空壳工具的完整实现（先决定"下线"或"排期"）

## 5. 成功标准（验收口径）

| 维度 | 指标 | 目标值 |
|---|---|---|
| 质量门禁 | 单测 | 全绿（基线：539 passed / 1 skipped；CI 在 ubuntu + windows 双平台） |
| 质量门禁 | 覆盖率 | ≥ 当前基线 64%，核心模块 ≥ 75% |
| 质量门禁 | lint | ruff 0 告警 |
...
| D3 | 空壳工具（Browser / MCP / Database） | 文档可信度与用户预期 | 先"下线或标注 Beta"，实现排到 M1 之后 |
