...
### 文档
- **全量同步（2026-09-14）**：把"文档承诺"与实现逐项对齐。
  - README：目标类型 `6 → 10`（并列出全部取值）、规划器单轮任务数改为"上限 8"、测试数与
    测试文件数更新为 572 / 49；删除已不成立的旧表述。
  - PROJECT_CHARTER：工具数 `9 → 6`、单测基线更新为 572，非目标项与决策表 D3 标注已完成。
  - CONTRIBUTING：单测基线 `526 → 572`，覆盖率要求明确为 64%（与 CI 门禁一致）。
  - docs/product/status.md：按最新实现重写对账表（意图/规划/回答/记忆/工具安全/构建配置等
    共 10 余项由"有差距"改为"已对齐"），剩余漂移仅前端未接线功能两项。
  - docs/risk-register.md：R-01（安全四项）、R-02（质量门禁）状态改为已关闭并附证据；
    R-07/R-08 按当前进度更新。
  - docs/product/backlog.md：P1-DOC-2（README 计数）、P1-DOC-3（截图）标记完成。
  - ROADMAP：版本升至 v1.1，M1 已完成项与剩余项重新标注。
  - docs/architecture.md：修正"评剧"错别字；补充运行时数据目录覆盖说明。
  - docs/deployment.md：补充 `WORKSPACE_ALLOWED_ROOTS` 与四个数据目录覆盖变量。
  - docs/reviews/2026-08-14-full-review.md：加快照说明（正文保持原样，修复进度指向 backlog）。
- CI 覆盖率门禁从 50% 提到 **64%**，与 DoD 中"覆盖率不低于基线"的声明一致（当前实测约 66%）。
